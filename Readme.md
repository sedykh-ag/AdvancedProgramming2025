...
- 1 балл. Перепишите код установления соединения со вторым клиентом на пересылку зарезервированных сообщений. Будьте внимательны к тому, что polling поток должен потокобезопасно сообщать о сетевых событиях в главный поток.

Опция А (простая)
- 1 балл. Сделайте детерминированную генерацию уровня и начните спавнить 2 сущности игроков. Синхронизируйте клиентов пересылкой input'а. Прикрепите короткое видео демонанстрации старта клиентов и их синхронизации.

Опция Б (сложная, но очень интересная)
- 4 балла. Реализуйте Клиент-Серверную архитектуру. Пусть исполняемый файл можно запустить в 2 режимах: при передаче аргумента --user_id=1/2 стартует клиент, при отсутствии аргументов или если есть --is_dedicated, то запускается сервер.
Сервер симулирует игру локально и посылает снапшоты всего состояния мира раз в кадр клиентам. А от клиентов слушает их Input.
Клиенты же должны принимать все события от сервера и обновлять мир соответсвующим образом. И отправлять свой Input на сервер.
Поскольку в качестве снапшотов предлагается посылать весь мир, то достаточно будет реализовать сериализацию и десериализацю архетипов. Не обязательно сериализовывать их по-объектно. 
Если будет проблема с производительностью, то можно уменьшить размеры мира.

Прикрепите короткое видео демонанстрации старта клиентов и сервера и их синхронизации.
